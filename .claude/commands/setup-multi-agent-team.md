...
for agent in alpha beta gamma delta; do
  ...
  worktree_path="../${PROJECT_NAME}-agent-${agent}"
  ...
  git config user.email "agent.${agent}@${PROJECT_NAME}.dev"
  cd - > /dev/null
...
# ${PROJECT_NAME} - Multi-Agent Development Sprint

## 🚀 Mission
Collaborative development using specialized agent roles.

## 📁 Project Structure
- **Main Repo**: \${PROJECT_ROOT}
- **Tech Stack**: [Customize for your project]
...
for agent in Alpha Beta Gamma Delta; do
  ...
  worktree_path="../\${PROJECT_NAME}-agent-\$agent_lower"
  ...
  if [ -d "\$worktree_path" ]; then
    ...
    echo "| \$agent | \$status | \$branch | \$commits | \$files | \$last_time |" >> "\$PROJECT_ROOT/\$DASHBOARD"
    cd - > /dev/null
  else
    echo "| \$agent | ❌ Missing | - | - | - | - |" >> "\$PROJECT_ROOT/\$DASHBOARD"
  fi
done
